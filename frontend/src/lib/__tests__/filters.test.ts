...
describe("filterExercises (T-FILT)", () => {
  ...
  it("T-FILT-02: filter by level returns correct subset", () => {
    ...
    expect(ids(result)).toEqual(["binary-search", "bubble-sort", "climbing-stairs", "fibonacci", "inorder-traversal", "linked-list", "palindrome-check", "remove-duplicates-dll", "remove-duplicates-sll", "reverse-string", "running-average", "two-sum", "valid-parentheses"]);
  });
  ...
  it("T-FILT-05: no-match filter returns empty array", () => {
    const result = filterExercises(exercises, {
      categories: [ExerciseCategory.BIT_MANIPULATION],
    });
